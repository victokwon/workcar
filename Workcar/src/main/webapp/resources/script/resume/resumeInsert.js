$(function () {
 ...
 // 자격증 1
  ...
  $("#addBtn2").on("click", function () {
    ...
    html += '<input type="button" class="minus_btn" id="delBtn" value="－">';
    html += '<div class="data_container">';
    html += '<select name="foreignLv" id="foreignLv">';
    html += '<option value="-1">언어 선택</option>';
    html += '<option value="0">영어</option>';
    html += '<option value="1">일본어</option>';
    html += '<option value="2">중국어</option>';
    html += '<option value="2">독일어</option>';
    html += '<option value="2">불어</option>';
    html += '<option value="2">스페인어</option>';
    html += '<option value="2">러시아어</option>';
    html += '<option value="2">이탈리아어</option>';
    html += '<option value="2">에스파냐어</option>';
    html += '<option value="2">아랍어</option>';
    html += '<option value="2">기타</option>';
    html += "</select>";
    html += '<label for="RLv">읽기/쓰기</label>';
    html += '<select name="RLv" id="RLv">';
    html += '<option value="-1">수준 선택</option>';
    html += '<option value="0">상</option>';
    html += '<option value="1">중</option>';
    html += '<option value="2">하</option>';
    html += " </select>";
    html += '<label for="SLv">회화</label>';
    html += '<select name="SLv" id="SLv">';
    html += '<option value="-1">수준 선택</option>';
    html += '<option value="0">상</option>';
    html += '<option value="1">중</option>';
    html += '<option value="2">하</option>';
