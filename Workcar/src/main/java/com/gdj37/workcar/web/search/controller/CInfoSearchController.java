...
//		System.out.println(params);
	...
	@RequestMapping(value = "/CorpViewsTopList")
	public ModelAndView CorpViewsTopList(@RequestParam HashMap<String, String> params, ModelAndView mav)throws Throwable {
		int page = 1;
		...
		mav.addObject("page", page);
		mav.setViewName("TOPRANK/CorpViewsTopList");

		return mav;
	}
	@RequestMapping(value = "/CorpViewsTopchListAjax", method = RequestMethod.POST, produces = "text/json;charset=UTF-8")
	@ResponseBody
	public String CorpViewsTopchListAjax(@RequestParam HashMap<String, String> params) throws Throwable {
		ObjectMapper mapper = new ObjectMapper();
//		System.out.println(params);
		Map<String, Object> modelMap = new HashMap<String, Object>(); // 데이터를 담을 map
		try {
			...
			int cnt = iCInfoSearchService.getCInfoSch1Cnt(params);
			// 페이징 정보 취득
			PagingBean pb = iPagingService.getPagingBean(page, cnt, 100, 1);
			params.put("startCnt", Integer.toString(pb.getStartCount()));
			params.put("endCnt", Integer.toString(pb.getEndCount()));

			// 리스트 조회
			List<HashMap<String, String>> list = iCInfoSearchService.getViewsTop1List(params);
